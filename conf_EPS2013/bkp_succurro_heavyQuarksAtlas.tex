...
\begin{frame}\frametitle{VLQ production}
\scriptsize\centering
...
\begin{frame}\frametitle{VLQ decay modes}
\scriptsize\centering
\myskip
...
{\usebeamercolor[fg]{head/foot boxes}
\pgfputat{\pgfxy(-2.2,1.75)}{\pgfbox[left,base]{\textbf{\large vector-like $T$}}}
\pgfputat{\pgfxy(4.,1.75)}{\pgfbox[left,base]{\textbf{\large vector-like $B$}}}
}
    \end{pgftranslate}
      \only<2>{
    \begin{pgftranslate}{\pgfpoint{0.35\textwidth}{0.65\textheight}}
...
\pgfputat{\pgfxy(6.7,0.0)}{\pgfbox[left,base]{\scriptsize $^{(a)}$BR($T/B\to  Zt/b$) = 1 }}
\pgfputat{\pgfxy(8.8,-0.7)}{\pgfbox[left,base]{\scriptsize - BR($T/B\to Ht/b$) }}
\pgfputat{\pgfxy(8.8,-0.4)}{\pgfbox[left,base]{\scriptsize - BR($T/B\to Wb/t$)}}
...
\footnotesize\centering
    ...
    OR 
    \begin{itemize}
    \item $\geq$4 jets and one $W^{type II}_{had}$ and no $W^{type I}_{had}$%\onslide<3->{
    \item $\geq$ 1 $b$tagged jet
    \item $H_T>750~$GeV
    \item $\Delta R(l,\nu) < 1.4$
    \item $min(\Delta R(l,b_{1,2})) > 1.4$
